...
fastdocs.prototype.headings = function(code) {
	...
	for(var i = 0; i < fast.tokens.length; i++)
	{
		if(fast.tokens[i].type == "heading")
		{
			...
			fast.TOCArray['#' + fast.current_file.split('.')[0] + '/' + fast.tokens[i].text.toLowerCase().replace(/[^\w]+/g, '-')] = header_level; 
...
// overwrite the header functinos of marked
fast.markdown.Renderer.prototype.heading = function(text, level, raw) {
  return '<h'
    + level
    + ' id="'
